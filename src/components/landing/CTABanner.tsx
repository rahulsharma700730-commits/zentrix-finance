    ...
    <section className="py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center max-w-3xl mx-auto">
          ...
          <p className="text-white/50 mb-8 text-lg">{t('cta.desc')}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" className="bg-gradient-gold text-[#0a0a0a] hover:opacity-90 text-base px-10 py-7 shadow-gold-lg font-semibold" onClick={() => navigate('/auth?tab=signup')}>
              {t('cta.create')} <ArrowRight className="ml-2 h-5 w-5" />
            </Button>
            <button className="inline-flex items-center justify-center rounded-md text-base px-10 py-3.5 font-medium border border-white/20 text-white hover:bg-white/10 transition-colors" onClick={() => navigate('/auth')}>
              {t('cta.loginDash')}
            </button>
          </div>
        </motion.div>
