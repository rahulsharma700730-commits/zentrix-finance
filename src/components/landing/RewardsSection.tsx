...
export const RewardsSection = () => {
  ...
  const pillars = [
    { icon: TrendingUp, k: 'pillar1', metric: '0.333%' },
    { icon: Users, k: 'pillar2', metric: '20%' },
    { icon: Trophy, k: 'pillar3', metric: 'Bronze → Crown' },
  ];
  return (
      ...
      <div className="container mx-auto px-4 relative">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <span className="inline-flex items-center gap-2 text-sm font-medium text-primary tracking-widest uppercase mb-3">
            <Sparkles className="w-4 h-4" /> {t('rewards.eyebrow')}
          </span>
          <h2 className="text-3xl md:text-5xl font-display font-bold mb-4">
            {t('rewards.title1')} <span className="text-gradient-gold">{t('rewards.title2')}</span> {t('rewards.title3')}
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">{t('rewards.desc')}</p>
        </motion.div>
          ...
          {pillars.map((p, i) => (
            <Card key={i} className="lift glass border-primary/15 overflow-hidden group">
              <CardContent className="p-7">
                ...
                <h3 className="font-display font-bold text-xl mb-1">{t(`rewards.${p.k}.title`)}</h3>
                <div className="flex items-baseline gap-2 mb-3">
                  <span className="text-3xl font-display font-bold text-gradient-gold">{p.metric}</span>
                  <span className="text-xs text-muted-foreground">{t(`rewards.${p.k}.sub`)}</span>
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed">{t(`rewards.${p.k}.body`)}</p>
              </CardContent>
            </Card>
          ))}
        </div>
          ...
          <Card className="glass-strong border-primary/15 ring-gold-glow overflow-hidden">
            <CardContent className="p-6 md:p-10">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-display font-bold text-2xl">{t('rewards.level.title')}</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-8">{t('rewards.level.desc')}</p>

              <div className="grid grid-cols-5 gap-2 md:gap-4">
                {levels.map((l) => (
                  <div key={l.lvl} className="text-center">
                    <div className="relative mb-3">
                      <div className={`mx-auto rounded-full bg-gradient-to-br ${l.color} flex items-center justify-center font-display font-bold text-primary-foreground shadow-gold`}
                        style={{ width: `${44 + l.rate * 4}px`, height: `${44 + l.rate * 4}px`, fontSize: `${14 + l.rate * 0.4}px` }}>
                        {l.rate}%
                      </div>
                    </div>
                    <div className="text-xs md:text-sm font-semibold">{t('rewards.level.levelShort')}{l.lvl}</div>
                    <div className="text-[10px] md:text-xs text-muted-foreground">{t('rewards.level.levelLong')} {l.lvl}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-10 p-5 rounded-xl bg-primary/5 border border-primary/15">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm leading-relaxed">
                    <strong className="text-foreground">{t('rewards.example.label')}</strong>{' '}
                    {t('rewards.example.before')}{' '}
                    <span className="text-primary font-semibold">{t('rewards.example.earns')}</span>
                    {t('rewards.example.middle')}{' '}
                    <span className="text-primary font-semibold">{t('rewards.example.youGet')}</span>{' '}
                    {t('rewards.example.after')}{' '}
                    <span className="text-primary font-semibold">{t('rewards.example.total')}</span>{' '}
                    {t('rewards.example.tail')}
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h3 className="font-display font-bold text-2xl mb-2">{t('rewards.ladder.title')}</h3>
            <p className="text-sm text-muted-foreground">{t('rewards.ladder.desc')}</p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4 stagger">
            {ranks.map((r, i) => (
              <div key={r.name} className="relative glass border border-primary/15 rounded-xl p-4 lift text-center" style={{ opacity: 0.6 + i * 0.08 }}>
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-gold" style={{ opacity: 0.4 + i * 0.15 }} />
                ...
                <div className="text-[11px] text-muted-foreground space-y-0.5 mt-2">
                  <div>{r.directs} {t('rewards.ladder.directs')}</div>
                  <div>{r.team} {t('rewards.ladder.team')}</div>
                  <div className="text-primary font-semibold">{r.volume} {t('rewards.ladder.volume')}</div>
                </div>
              </div>
            ))}
          </div>
